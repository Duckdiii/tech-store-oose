...
export function WarehouseWorkspace({ view, navigate, products = [], variants = [] }) {
  const tabs = [['overview', 'Tổng quan'], ['import', 'Nhập kho'], ['export', 'Xuất kho'], ['logs', 'Warehouse Log']];

  return <>
    <style>{warehouseStyles}</style>
    <div className="warehouse-tabs">{tabs.map(([key, label]) => <button key={key} className={view === key ? 'is-active' : ''} onClick={() => navigate(`/admin/warehouse${key === 'overview' ? '' : `/${key}`}`)}>{label}</button>)}</div>
...
function WarehouseOverview({ navigate, products = [], variants = [] }) {
  ...
  return <>
    <div className="admin-page-intro"><div><p>QUẢN LÝ KHO</p><h2>Tổng quan</h2></div><button className="admin-button admin-button--secondary" onClick={() => navigate('/admin/products')}>Quản lý Product →</button></div>
    {products.length > 0 ? (
                  ...
                  <>
                    <tr key={product.id} className="warehouse-product-row" style={{ borderTop: '1px solid #f0f1f3', transition: 'background-color 0.2s' }} onClick={() => setExpandedProduct(isExpanded ? null : product.id)}>
                      <td style={{ padding: '12px', textAlign: 'center' }}>
                        <button
                          onClick={(event) => { event.stopPropagation(); setExpandedProduct(isExpanded ? null : product.id); }}
                          style={{
                            background: 'none',
                            border: 'none',
                            cursor: 'pointer',
                      ...
                      <td style={{ padding: '12px', textAlign: 'right' }}><button className="admin-row-action" onClick={(event) => { event.stopPropagation(); navigate('/admin/products'); }}>Quản lý</button></td>
                    </tr>
                    {isExpanded && (
                      <tr key={`details-${product.id}`} style={{
                        animation: 'slideDown 0.35s cubic-bezier(0.16, 1, 0.3, 1) forwards',
                                    ...
                                    <tr key={variant.id} style={{ borderTop: '1px solid #eee', fontSize: '12px' }}>
                                      ...
                                      <td style={{ padding: '10px 12px', textAlign: 'center' }}>
                                        <span style={{
                                          ...
                                          fontSize: '12px',
                                          fontWeight: '700',
                                          backgroundColor: isAvailable ? '#d4edda' : '#fff1f0',
                                          color: isAvailable ? '#155724' : '#9b1c1c',
                                          border: isAvailable ? '1px solid rgba(21,87,36,0.08)' : '1px solid rgba(139, 18, 18, 0.06)'
                                        }}>
...
function ImportFlow({ products = [], variants = [] }) {
  ...
  const applyFirstSerialToAll = () => {
    setItems((current) => {
      if (current.length < 2) return current;
      const { serialId, ...sharedInfo } = current[0];
      return current.map((item, index) => index === 0 ? item : { ...item, ...sharedInfo });
    });
  };
  const requestPayload = () => ({
    productId: mode === 'existing' ? productId.trim() || null : null,
    newProduct: mode === 'new' ? { ...newProduct, name: newProduct.name.trim(), description: newProduct.description.trim() || null, brandId: newProduct.brandId.trim(), categoryId: newProduct.categoryId.trim() } : null,
    note: note.trim() || null,
    items: items.map((item) => ({
      serialId: item.serialId.trim(), ramGb: item.ramGb === '' ? null : Number(item.ramGb), storageGb: item.storageGb === '' ? null : Number(item.storageGb), color: item.color.trim() || null, price: Number(item.price), importPrice: Number(item.importPrice),
    })),
  });
  const isReady = () => {
    if (mode === 'existing' && !productId.trim()) return false;
    if (mode === 'new' && (!newProduct.name.trim() || !newProduct.brandId.trim() || !newProduct.categoryId.trim())) return false;
    return items.length > 0 && items.every((item) => item.serialId.trim() && Number(item.price) > 0 && Number(item.importPrice) > 0);
  };
  const validate = async (event) => {
    event.preventDefault(); setError(''); setResult(null);
    if (!isReady()) { setError('Nhập đủ thông tin sản phẩm, serial ID, giá bán và giá nhập trước khi validate.'); return; }
    // client-side uniqueness checks for serial IDs
    const serials = items.map((it) => it.serialId.trim()).filter(Boolean).map((s) => s.toUpperCase());
    const dupInList = serials.filter((s, idx) => serials.indexOf(s) !== idx);
    if (dupInList.length) { setError(`Serial ID trùng trong danh sách: ${[...new Set(dupInList)].join(', ')}`); return; }
    const existing = new Set((variants || []).map((v) => String(v.id || v.productVariantId || '').toUpperCase()));
    const colliding = serials.filter((s) => existing.has(s));
    if (colliding.length) { setError(`Serial ID đã tồn tại trong kho: ${[...new Set(colliding)].join(', ')}`); return; }
    setLoading(true);
    const payload = requestPayload();
    try { setPreview(await validateImport(payload)); setValidatedPayload(payload); }
    catch (requestError) { setPreview(null); setValidatedPayload(null); setError(getApiError(requestError)); }
    finally { setLoading(false); }
  };
  const confirm = async () => {
    setError(''); setLoading(true);
    try { setResult(await confirmImport(validatedPayload)); setPreview(null); }
...
function ExportFlow({ products = [], variants = [] }) {
  ...
  const filteredVariants = availableVariants.filter((variant) => {
    ...
    const matchesBrand = !brandFilter || product?.brand === brandFilter;
    const matchesPrice = (!minPrice || variant.price >= Number(minPrice)) && (!maxPrice || variant.price <= Number(maxPrice));
    return matchesProduct && matchesBrand && matchesPrice;
  });

  const selectedVariantRows = selectedSerials
    .map((serial) => availableVariants.find((variant) => variant.id.toLowerCase() === serial.toLowerCase()))
    .filter(Boolean);
  ...
  const validate = async (event) => {
    ...
    if (!request.serialIds.length) { setError('Chọn ít nhất một serial AVAILABLE để xuất kho.'); return; }
    setLoading(true);
    try { setPreview(await validateExport(request)); setValidatedPayload(request); }
    catch (requestError) { setPreview(null); setValidatedPayload(null); setError(getApiError(requestError)); }
    finally { setLoading(false); }
  };
  ...
  const selectedQuantityLabel = selectedCount === 0 ? 'Chưa chọn sản phẩm nào' : `Đã chọn: ${selectedCount} serial`;
    ...
    <section className="warehouse-flow">
      ...
      <form onSubmit={validate} className="admin-card warehouse-form warehouse-export-panel">
        <div className="warehouse-export-picker">
          <div className="warehouse-export-summary">
            <div>
              <span>{selectedQuantityLabel}</span>
              <strong>{selectedCount}</strong>
              <small>Sản phẩm được chọn từ danh sách AVAILABLE</small>
            </div>
            <div>
              <small>{filteredVariants.length} sản phẩm có thể xuất</small>
            </div>
          </div>
          ...
          <div className="warehouse-export-grid">
            {filteredProducts.map((product) => {
              ...
              return (
                <article key={product.id} className="warehouse-export-product-card">
                  <button
                    type="button"
                    className="warehouse-export-product-head"
                    style={{
                      background: 'none',
                      border: 'none',
                      cursor: 'pointer',
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                      <span className="admin-status" style={{ whiteSpace: 'nowrap' }}>
                        {productVariants.length} serial AVAILABLE
                      </span>
                      <span
                        style={{
                          fontSize: '18px',
                  ...
                  {isExpanded && (
                    <div className="warehouse-export-product-body">
                      {productVariants.map((variant) => {
                        const checked = selectedSerials.some(
                          (serial) => serial.toLowerCase() === variant.id.toLowerCase()
                        );
                        return (
                          <label key={variant.id} className="warehouse-export-serial-row">
                            <input
                              ...
                              onChange={() =>
                                setSelectedSerials((current) =>
                                  current.some((serial) => serial.toLowerCase() === variant.id.toLowerCase())
                                    ? current.filter((serial) => serial.toLowerCase() !== variant.id.toLowerCase())
                                    : [...current, variant.id]
                                )
                              }
                            />
                            <div className="warehouse-export-serial-main">
                              <b>{variant.id}</b>
                              <small>
                                {variant.ramGb} GB RAM · {variant.storageGb} GB · {variant.color}
                              </small>
                            </div>
            ...
            {filteredVariants.length === 0 && (
              <div className="warehouse-export-empty">Không có serial AVAILABLE phù hợp bộ lọc hiện tại.</div>
            )}
          </div>
        </div>
        ...
        <div className="warehouse-export-selected-list">
          {selectedVariantRows.length > 0 ? (
            ...
            >
              📋 Xem danh sách xuất hàng ({selectedVariantRows.length} serial)
            </button>
          ) : (
            <div className="warehouse-export-empty">Chưa chọn sản phẩm nào.</div>
          )}
        </div>
        ...
        <div className="warehouse-actions">
          <button className="admin-button" disabled={loading}>
            {loading ? 'Đang kiểm tra...' : 'Kiểm tra trước khi xuất'}
          </button>
        </div>
      </form>
      ...
      {preview && (
        <div className="warehouse-preview">
          <p>KIỂM TRA XUẤT KHO THÀNH CÔNG</p>
          <h3>{preview.exportQuantity} serial sẵn sàng xuất</h3>
          <span>Backend xác nhận các ProductVariant đều đang AVAILABLE.</span>
          <div>
            <button className="admin-button" disabled={loading} onClick={confirm}>
              {loading ? 'Đang xuất...' : 'Confirm export'}
            </button>
      ...
      {result && (
        <div className="warehouse-result">
          <ApiMessage success={`${result.message}. Mã ExportLog: ${result.exportLogId}.`} />
          {result.receipt && (
            ...
            <div className="warehouse-status-list">
              {result.inventoryStatuses.map((status) => (
                <div key={status.productId}>
                  ...
                  <span>
                    {status.availableQuantity} AVAILABLE · {status.status}
                  </span>
                </div>
              ))}
            </div>
          ...
          {result.warnings?.map((warning) => (
            <ApiMessage key={warning} error={warning} />
          ))}
        </div>
      )}
          ...
          >
            ...
            >
              <div>
                ...
                <h3 style={{ margin: 0, color: '#0d1117', fontSize: '18px' }}>
                  {selectedVariantRows.length} sản phẩm xuất hàng
                </h3>
              </div>
              <button
                type="button"
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: '16px 20px' }}>
              {selectedVariantRows.map((variant, index) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      padding: '12px 0',
                      borderBottom: index < selectedVariantRows.length - 1 ? '1px solid #edf1f5' : 'none',
                      display: 'flex',
                      justifyContent: 'space-between',
                  ...
                  >
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        {product?.name || variant.productId} · {variant.ramGb} GB RAM / {variant.storageGb} GB ·{' '}
                        {variant.color}
                      </small>
                    </div>
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      <div style={{ fontWeight: '600', color: '#0d1117' }}>{formatMoney(variant.price)}đ</div>
                      <small style={{ color: '#64748b', fontSize: '11px' }}>AVAILABLE</small>
                    </div>
                  </div>
                );
              })}
