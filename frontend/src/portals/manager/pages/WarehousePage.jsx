...
function ImportFlow({ products = [], variants = [] }) {
  ...
  const applyFirstSerialToAll = () => { setItems((current) => { if (current.length < 2) return current; const { serialId: _, ...sharedInfo } = current[0]; return current.map((item, index) => index === 0 ? item : { ...item, ...sharedInfo }); }); };
