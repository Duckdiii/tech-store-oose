...
function ImportFlow({ products = [], variants = [] }) {
  ...
  const applyFirstSerialToAll = () => { setItems((current) => { if (current.length < 2) return current; const sharedInfo = { ramGb: current[0].ramGb, storageGb: current[0].storageGb, color: current[0].color, price: current[0].price, importPrice: current[0].importPrice }; return current.map((item, index) => index === 0 ? item : { ...item, ...sharedInfo }); }); };
