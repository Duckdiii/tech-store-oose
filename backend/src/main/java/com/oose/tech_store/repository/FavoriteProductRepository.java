...
public interface FavoriteProductRepository extends JpaRepository<FavoriteProduct, String> {
    ...
    @Query("SELECT f FROM FavoriteProduct f WHERE f.productVariant.product.id = :productId " +
            "AND (:ramGb IS NULL OR f.productVariant.ramGb = :ramGb) " +
            "AND (:storageGb IS NULL OR f.productVariant.storageGb = :storageGb) " +
            "AND (:color IS NULL OR f.productVariant.color = :color) " +
            "AND f.status = :status")
